...
def example_basic_treenode_operations():
    ...
    print(f"   Child nodes created: {engineering.name}, {marketing.name}")
    
    # Add children
    print("\n2. Adding children:")
    root.add_child(engineering)
    root.add_child(marketing)
    print(f"   Root now has {len(root.children)} children")
    print(f"   Engineering parent: {engineering.parent.name}")
    
    # Get children
    print("\n3. Getting children:")
    first_child = root.get_child(0)  # By index
    eng_child = root.get_child("Engineering")  # By name
    print(f"   First child by index: {first_child.name}")
    print(f"   Engineering child by name: {eng_child.name}")
    ...
    print("\n4. Tree structure so far:")
    draw_tree(root)
    
    print(f"\n5. Node statistics:")
    print(f"   Root max depth: {root.max_depth()}")
    print(f"   Root max width: {root.max_width()}")
    print(f"   Engineering max depth: {engineering.max_depth()}")
...
def example_treenode_content_operations():
    ...
    print(f"   String content: {string_node.content}")
    print(f"   Dict content keys: {list(dict_node.content.keys())}")
    print(f"   List content length: {len(list_node.content)}")
    print(f"   Numeric content: {numeric_node.content}")
    print(f"   None content: {none_node.content}")
...
def example_treenode_search_operations():
    ...
    print(f"   Found NonExistent: {found_deep.name if not_found else 'Not found'}")
...
def example_tree_class_operations():
    ...
    print("1. Initial tree (just root):")
    draw_tree(tree.root)
...
def main():
    ...
    print("=" * 60)
    print("This concludes the comprehensive demonstration of all")
    print("classes and functions in the pytree module.")
    print("\nKey takeaways:")
